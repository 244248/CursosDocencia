...
const AdminPanel = () => {
  ...
  const [expandedSections, setExpandedSections] = useState({ cursos: true, docentes: true, estadisticas: false });
  ...
  const activityLog = useMemo(() => {
    ...
    teachers.forEach((teacher) => {
      ...
      const details = [
        ...
        teacher.matricula && `Matrícula: ${teacher.matricula}`,
    ...
    pendingTeachers.forEach((pending) => {
      ...
      const details = [
        ...
        pending.matricula && `Matrícula: ${pending.matricula}`,
  ...
  const saveCourse = async (courseData, courseKey = null) => {
    try {
      ...
      if (courseKey) {
        ...
        DualWriteService.saveCurso(courseKey, courseDataWithStatus);
        ...
        DualWriteService.saveCurso(newCourseKey, courseDataWithStatus);
  ...
  const registerTeacher = async (teacherData) => {
    try {
      ...
      await set(ref(rtdb, `docentes/${teacherKey}`), docentePayload);
      DualWriteService.saveDocente(teacherKey, docentePayload);
  ...
  const handleSendReminders = async () => {
    ...
    try {
      const results = await ReminderService.enviarRecordatorios(courses, teachers);
      setReminderResults(results);
      setShowReminderModal(true);
    } catch (error) {
      showMessage('Error al enviar recordatorios: ' + error.message, 'error');
    }
  ...
  const handleTeacherSubmit = async (e) => {
    ...
    if (!teacherData.matricula.trim()) {
      showMessage("La matrícula es obligatoria", "error");
      return;
    }
  ...
  const downloadAttendancePDF = (courseKey) => {
    try {
      ...
      attendingTeachers.forEach((teacher, index) => {
        ...
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(10);
        pdf.text(`   Matrícula: ${teacher.matricula}`, 30, yPosition);
        yPosition += 6;
        ...
        <table className="courses-table">
          ...
          <tbody>
            {filteredCourses.map(([courseKey, course], index) => {
              ...
              return (
                <tr key={courseKey} className="course-table-row">
                  <td className="course-number" data-label="No.">{index + 1}</td>
                  <td className="course-name" data-label="Curso">{course.nombre}</td>
                  <td className="course-type" data-label="Tipo">
                    <span className="course-type-badge">{formatCourseType(course.tipo)}</span>
                  </td>
                  <td className="course-url-date" data-label="URL/Fecha">{urlOrDate}</td>
                  <td className="course-teachers" data-label="Docentes">
                  ...
                  <td className="course-actions-cell" data-label="Acciones">
                    <div className="table-actions">
                      ...
                      <button 
                        ...
                        onClick={() => downloadAttendancePDF(courseKey)}
                      ...
                      <button 
                        ...
                        onClick={() => confirmDelete(courseKey)}
                        title="Eliminar"
                      >
        ...
        <table className="teachers-table">
          ...
          <tbody>
            {filteredTeachers.map((teacher, index) => {
              ...
              return (
                <tr key={teacher.key} className="teacher-table-row">
                  <td className="teacher-number" data-label="No.">{index + 1}</td>
                  <td className="teacher-name" data-label="Nombre">{teacher.nombre}</td>
                  <td className="teacher-email" data-label="Email">{teacher.email}</td>
                  <td className="teacher-matricula" data-label="Matrícula">{teacher.matricula}</td>
                  <td className="teacher-area" data-label="Área">
                    <span className="teacher-area-badge">{formatArea(teacher.area)}</span>
                  </td>
                  <td className="teacher-courses-assigned" data-label="Cursos">
                  ...
                  <td className="teacher-actions-cell" data-label="Acciones">
                    <div className="table-actions">
                      <button 
                        className="btn-action btn-view" 
                      ...
                      <button 
                        className="btn-action btn-delete" 
                        onClick={() => confirmDeleteTeacher(teacher.key)}
              ...
              {isExpanded && (
                <div className="teacher-details">
                  <ul className="teacher-info-list">
                    ...
                    <li>
                      <strong>Matrícula:</strong> {teacher.matricula}
                    </li>
                    <li>
  ...
  const renderAgentIA = () => {
    return (
      <div className="agent-ia-container">
        <VoiceAgent />
      </div>
    );
  };
          ...
          {searchResults.map(teacher => {
            ...
            return (
              <div key={teacher.key} className="teacher-item">
                ...
                <p><strong>Matrícula:</strong> {teacher.matricula}</p>
      ...
      <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''} ${sidebarMobileOpen ? 'mobile-open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          <div className="sidebar-section">
            <div className="sidebar-section-header" onClick={() => toggleSection('cursos')}>
              <h3 className="sidebar-section-title"><i className="fas fa-book"></i> Cursos</h3>
              <i className={`fas fa-chevron-${expandedSections.cursos ? 'up' : 'down'} sidebar-section-toggle`}></i>
            </div>
            <ul className={`sidebar-nav ${expandedSections.cursos ? 'expanded' : 'collapsed'}`}>
          ...
          <div className="sidebar-section">
            <div className="sidebar-section-header" onClick={() => toggleSection('docentes')}>
              <h3 className="sidebar-section-title"><i className="fas fa-chalkboard-teacher"></i> Docentes</h3>
              <i className={`fas fa-chevron-${expandedSections.docentes ? 'up' : 'down'} sidebar-section-toggle`}></i>
            </div>
            <ul className={`sidebar-nav ${expandedSections.docentes ? 'expanded' : 'collapsed'}`}>
              <li className="sidebar-nav-item">
                <a 
                  href="#" 
          ...
          <div className="sidebar-section">
            <div className="sidebar-section-header" onClick={() => toggleSection('estadisticas')}>
              <h3 className="sidebar-section-title"><i className="fas fa-chart-pie"></i> Estadísticas</h3>
              <i className={`fas fa-chevron-${expandedSections.estadisticas ? 'up' : 'down'} sidebar-section-toggle`}></i>
            </div>
            <ul className={`sidebar-nav ${expandedSections.estadisticas ? 'expanded' : 'collapsed'}`}>
              <li className="sidebar-nav-item">
                <a 
                  href="#" 
          ...
          <div className="sidebar-section sidebar-section-divider">
            <a 
              href="#" 
              className={`sidebar-nav-link sidebar-ai-link ${activeSection === 'agenteIA' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); handleSectionChange('agenteIA'); }}
            >
              <i className="fas fa-robot sidebar-nav-icon"></i>
              <span className="sidebar-nav-text">Agente IA</span>
            </a>
          </div>
        </nav>
      </aside>
        ...
        <header className="main-header">
          ...
          <div className="header-left">
            <h1 className="header-title">
              ...
              {activeSection === 'agenteIA' && 'Agente IA'}
            </h1>
          </div>
            ...
            >
              {currentAdmin?.fotoUrl ? (
                ...
                <div className="user-avatar-placeholder">
                  {(currentAdmin?.nombre || 'A').charAt(0).toUpperCase()}
                </div>
              )}
            </div>
          </div>
        ...
        <section className="content-area">
          ...
          <div className="action-buttons-container">
            <div className="action-buttons">
              ...
              <button className="btn btn-warning" onClick={handleSendReminders} disabled={sendingReminders}>
                <i className={`fas ${sendingReminders ? 'fa-spinner fa-spin' : 'fa-bell'}`}></i>
                <span>{sendingReminders ? 'Enviando...' : 'Enviar Recordatorios'}</span>
              </button>
            </div>
          </div>
      ...
      {showReminderModal && reminderResults && (
        <div className="modal-backdrop" onClick={() => setShowReminderModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2><i className="fas fa-bell"></i> Recordatorios Enviados</h2>
              <button className="close" onClick={() => setShowReminderModal(false)}>
                <i className="fas fa-times"></i>
              </button>
            </div>
            <div className="modal-body">
              {reminderResults.mensaje ? (
                <div className="reminder-empty">
                  <i className="fas fa-check-circle" style={{fontSize: '3rem', color: '#27ae60', marginBottom: '1rem'}}></i>
                  <p>{reminderResults.mensaje}</p>
                </div>
              ) : (
                <>
                  <div className="reminder-summary">
                    <div className="reminder-stat success">
                      ...
                      <span>{reminderResults.enviados} enviados</span>
                    </div>
                    {reminderResults.errores > 0 && (
                      <div className="reminder-stat error">
                        <i className="fas fa-exclamation-circle"></i>
                        <span>{reminderResults.errores} errores</span>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="reminder-details">
                      ...
                      <ul className="reminder-list">
                        {reminderResults.detalles.map((d, i) => (
                          <li key={i} className={`reminder-item ${d.status}`}>
                            <i className={`fas ${d.status === 'enviado' ? 'fa-check' : 'fa-times'}`}></i>
                            <span><strong>{d.email}</strong> - {d.curso}</span>
            ...
            <div className="modal-actions">
              <button className="btn btn-primary" onClick={() => setShowReminderModal(false)}>
                Cerrar
              </button>
            </div>
          </div>
          ...
          <div className="modal-content teacher-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <form onSubmit={handleTeacherSubmit}>
                ...
                <div className="form-group">
                  <label htmlFor="teacherMatricula">Matrícula *</label>
