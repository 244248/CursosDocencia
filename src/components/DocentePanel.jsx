...
const DocentePanel = () => {
  ...
  const updateCourseStatus = async (courseId, teacherId, isOnline, statusType) => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        showMessage(
          isOnline
            ? statusType === 'complete' ? "Curso marcado como completado" : "Estado de curso actualizado"
            : statusType === 'confirm' ? "Asistencia confirmada" : "Asistencia cancelada",
          "success"
        );
      } else {
  ...
  const summary = useMemo(() => {
    if (!currentTeacher || assignedCourses.length === 0) {
      return { total: 0, completed: 0, pending: 0, online: 0, presential: 0, upcoming: null };
    }

    let completed = 0, pending = 0, online = 0, presential = 0, upcoming = null;
    const now = new Date();

    assignedCourses.forEach(([courseId, course]) => {
      const isOnline = !!(course.url && course.url.match(/^https?:\/\/.+/));
      if (isOnline) online += 1; else presential += 1;
      ...
      if (course.fecha) {
        ...
        if (!isNaN(courseDate) && courseDate >= now) {
          if (!upcoming || courseDate < upcoming.date) {
            upcoming = { id: courseId, name: course.nombre, date: courseDate };
          }
        }
      }
    });

    return { total: assignedCourses.length, completed, pending, online, presential, upcoming };
  ...
  const handleCourseAction = (courseId, action) => {
    ...
    if (action === 'complete') updateCourseStatus(courseId, currentTeacher.key, true, 'complete');
    else if (action === 'confirm') updateCourseStatus(courseId, currentTeacher.key, false, 'confirm');
    else if (action === 'cancel') updateCourseStatus(courseId, currentTeacher.key, false, 'cancel');
  };

  const handleFilterChange = (filter) => setActiveFilter(filter);

  useEffect(() => {
    if (currentTeacher) loadCourses();
  }, [currentTeacher]);
  ...
  return (
    <>
        ...
        <div className="header-left">
          ...
          <div className="header-text">
            ...
            <div className="header-user-info">
              <div className="docente-avatar-container" onClick={() => setShowProfile(true)} title="Ver perfil">
                {currentTeacher?.fotoUrl ? (
                  <img src={currentTeacher.fotoUrl} alt={currentTeacher?.nombre || 'Docente'} className="docente-avatar" />
                ) : (
                  <div className="docente-avatar-placeholder"><i className="fas fa-user"></i></div>
                )}
              </div>
              <div>
                <h2 onClick={() => setShowProfile(true)} title="Ver perfil">
                  Bienvenido <span id="userName">{currentTeacher?.nombre || ''}</span>
                </h2>
                <p id="userArea">{currentTeacher ? `Área: ${formatArea(currentTeacher.area)}` : ''}</p>
              </div>
            </div>
          </div>
        </div>
        <div className="header-actions">
          <button className="btn chat-toggle-btn" onClick={() => setShowChatPanel(!showChatPanel)} title="Asistente IA">
      ...
      {showChatPanel && (
        <div className="docente-chat-panel">
          <VoiceAgent sidebarMode={true} />
        </div>
      )}
      ...
      <main className="docente-main">
        ...
        <div className="dashboard-controls">
          <div className="summary-grid">
            <article className="summary-card accent-primary">
              <div className="summary-icon"><i className="fas fa-layer-group"></i></div>
              <div>
            ...
            <article className="summary-card accent-success">
              ...
              <div>
                ...
                <span className="summary-value">{summary.total > 0 ? Math.round((summary.completed / summary.total) * 100) : 0}%</span>
            ...
            <article className="summary-card accent-warning">
              ...
              <div>
                ...
                <span className="summary-value">{summary.upcoming ? summary.upcoming.date.toLocaleDateString('es-MX', { day: '2-digit', month: 'short' }) : 'Sin fecha'}</span>
                <span className="summary-sub">{summary.upcoming ? summary.upcoming.name : 'No hay eventos próximos'}</span>
              </div>
            </article>
          </div>
        </div>
        ...
        <div className="course-filters">
          <div className="filters-header">
            <h3 className="filters-title"><i className="fas fa-filter"></i> Filtrar Cursos</h3>
          ...
          <div className={`filter-buttons ${showMobileFilters ? 'expanded' : ''}`}>
            <button className={`filter-btn ${activeFilter === 'all' ? 'active' : ''}`} onClick={() => handleFilterChange('all')}>
              <i className="fas fa-list"></i><span>Todos</span>
            </button>
            <button className={`filter-btn ${activeFilter === 'basicos' ? 'active' : ''}`} onClick={() => handleFilterChange('basicos')}>
              <i className="fas fa-graduation-cap"></i><span>Básicos</span>
            </button>
            <button className={`filter-btn ${activeFilter === 'tecnologias' ? 'active' : ''}`} onClick={() => handleFilterChange('tecnologias')}>
              <i className="fas fa-laptop-code"></i><span>Tecnologías</span>
            </button>
            <button className={`filter-btn ${activeFilter === 'avanzados' ? 'active' : ''}`} onClick={() => handleFilterChange('avanzados')}>
              <i className="fas fa-brain"></i><span>Avanzados</span>
            </button>
            <button className={`filter-btn ${activeFilter === 'especializacion' ? 'active' : ''}`} onClick={() => handleFilterChange('especializacion')}>
              <i className="fas fa-award"></i><span>Especialización</span>
            </button>
          </div>
        </div>
          ...
          <div className="courses-table-wrapper">
            {!currentTeacher ? (
              ...
              <div className="no-courses">
                ...
                <p>{activeFilter !== 'all' ? 'No hay cursos de este tipo' : 'No hay cursos asignados'}</p>
              </div>
            ) : (
                      ...
                      >
                        ...
                        <td className="course-name">
                          <div className="course-name-content">
                            ...
                            {course.descripcion && <span className="course-description">{course.descripcion}</span>}
                          </div>
                        </td>
                        <td><span className="course-type-badge">{formatCourseType(course.tipo)}</span></td>
                        ...
                        <td className="course-action">
                          <div className="action-buttons" onClick={(e) => e.stopPropagation()}>
                            {isOnline && !status.completed && (
                              <button className="action-icon-btn complete-btn" onClick={() => handleCourseAction(courseId, 'complete')} title="Completado">
                                <i className="fas fa-check"></i>
                              </button>
                            )}
                            {!isOnline && !status.willAttend && (
                              <button className="action-icon-btn confirm-btn" onClick={() => handleCourseAction(courseId, 'confirm')} title="Confirmar">
                                <i className="fas fa-calendar-check"></i>
                              </button>
                            )}
                            {!isOnline && status.willAttend && (
                              <button className="action-icon-btn cancel-btn" onClick={() => handleCourseAction(courseId, 'cancel')} title="Cancelar">
            ...
            <div className="course-modal-body">
              <div className="course-info-row">
                <div className="course-info-item">
                  ...
                  <div><strong>Tipo</strong><p>{formatCourseType(selectedCourse.tipo)}</p></div>
                </div>
              </div>
              {selectedCourse.area && (
                <div className="course-info-row">
                  <div className="course-info-item">
                    ...
                    <div><strong>Área</strong><p>{formatArea(selectedCourse.area)}</p></div>
                  </div>
                </div>
              )}
              {selectedCourse.fechaPublicacion && (
                <div className="course-info-row">
                  <div className="course-info-item">
                    ...
                    <div><strong>Publicación</strong><p>{new Date(selectedCourse.fechaPublicacion).toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p></div>
                  </div>
                </div>
              )}
              {selectedCourse.fecha && (
                <div className="course-info-row">
                  <div className="course-info-item">
                    ...
                    <div><strong>Fecha del Curso</strong><p>{new Date(selectedCourse.fecha).toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p></div>
                  </div>
                </div>
              )}
              {selectedCourse.descripcion && (
                <div className="course-info-row">
                  <div className="course-info-item">
                    ...
                    <div><strong>Descripción</strong><p>{selectedCourse.descripcion}</p></div>
                  </div>
                </div>
              )}
              {selectedCourse.url && selectedCourse.url.match(/^https?:\/\/.+/) && (
                <div className="course-info-row">
                  <div className="course-info-item">
                    ...
                    <div><strong>Enlace</strong><p><a href={selectedCourse.url} target="_blank" rel="noopener noreferrer" className="course-link">{selectedCourse.url}</a></p></div>
                  </div>
                </div>
              )}
            </div>
            <div className="course-modal-footer">
              {selectedCourse.url && selectedCourse.url.match(/^https?:\/\/.+/) && !selectedCourse.teacherStatus?.[currentTeacher?.key]?.completed && (
                <button className="btn mark-complete-btn" onClick={() => { handleCourseAction(selectedCourse.id, 'complete'); setShowCourseModal(false); }}>
                  <i className="fas fa-check"></i> Marcar como Completado
                </button>
              )}
              {(!selectedCourse.url || !selectedCourse.url.match(/^https?:\/\/.+/)) && !selectedCourse.teacherStatus?.[currentTeacher?.key]?.willAttend && (
                <button className="btn confirm-attend-btn" onClick={() => { handleCourseAction(selectedCourse.id, 'confirm'); setShowCourseModal(false); }}>
                  <i className="fas fa-calendar-check"></i> Confirmar Asistencia
                </button>
              )}
              {(!selectedCourse.url || !selectedCourse.url.match(/^https?:\/\/.+/)) && selectedCourse.teacherStatus?.[currentTeacher?.key]?.willAttend && (
                <button className="btn cancel-btn" onClick={() => { handleCourseAction(selectedCourse.id, 'cancel'); setShowCourseModal(false); }}>
            ...
            const loadUpdatedTeacher = async () => {
              try {
                ...
                if (teacherKey) {
                  const snapshot = await get(ref(rtdb, `docentes/${teacherKey}`));
                  const docenteData = snapshot.val();
                  if (docenteData) setCurrentTeacher({ ...docenteData, key: teacherKey, uid: auth.currentUser?.uid });
                }
              } catch (error) {
